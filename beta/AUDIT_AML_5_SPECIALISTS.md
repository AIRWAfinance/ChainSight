...
|---|---------|----------|-----|
| 1.1 | ~~No sanctions list freshness SLA exposed~~ **CLOSED 2026-04-29:** `getSanctionsFreshness()` exposes per-list `lastSyncedAt`, `ageHours`, `count`, `lastSyncOk`. `RiskReport.meta.sanctionsFreshness` carries snapshot per scan. `/api/sanctions/freshness` powers a `<SanctionsFreshnessBanner>` on the home page that turns red if any list >24h stale. | ✅ Closed | — |
| 1.2 | ~~First-match-wins hides multi-jurisdiction overlap~~ **CLOSED 2026-04-29:** `SanctionedAddress.lists: SanctionsList[]` now tracks every applicable jurisdiction. `list` retained for backwards-compat. Tests in `tests/sanctions-merge.test.ts` lock the merge semantics. | ✅ Closed | — |
| 1.3 | ~~Missing EU/UK/UN sync scripts and cron~~ **CLOSED 2026-04-29:** `scripts/sync-un.ts` added; all 4 sync scripts now emit ISO `lastSyncedAt`, `lastSyncOk`, `count`, `list`. Vercel Cron at `/api/cron/sync-sanctions` runs daily 02:00 UTC, protected by Vercel-Cron header OR `CRON_SECRET` Bearer token. `npm run sync:all` now includes UN. | ✅ Closed | — |
...
| # | Finding | Severity | Fix |
|---|---------|----------|-----|
| 2.1 | ~~Layering window 1h-only, ETH-only~~ **CLOSED 2026-04-29:** Detector rewritten with three windows (15min → high, 1h → medium, 24h → low) and asset-aware grouping. ERC-20 transactions matched per token contract; ETH and USDT on the same address produce two separate findings. Stablecoin-aware critical threshold (≥100k USDT/USDC notional vs ≥100 ETH). Tightest-window-wins matching ensures fast pass-through ranks above slow. Bonus: also fixed a latent decimals bug — `RawTx.tokenDecimal` + `NormalizedTransaction.tokenDecimal` now propagate, so non-18-decimal tokens (USDT/USDC = 6) report correct amounts. Tests in `tests/typologies.test.ts` (5 new). | ✅ Closed | — |
...
| 2.4 | ~~Severity weights are undocumented~~ **CLOSED 2026-04-29:** `/methodology/calibration` page documents the full formula, every severity weight (5/15/30/60), every typology multiplier (1.5/1.3/1.2/1.1/1.0/1.0/0.9), the verdict tiers, the public references behind each design choice (FATF VA RFI, FinCEN advisories, OFAC, EBA), AND honest limitations (current weights chosen by practitioner reading public guidance — calibration study against Elliptic/Chainalysis datasets is v0.8). Tests in `tests/calibration.test.ts` lock the documented values to the runtime scorer, forcing a change-management touchpoint if either drifts. | ✅ Closed | — |
...
| 5.4 | ~~No Trust statement / data residency~~ **CLOSED 2026-04-29:** `/trust` page published with 12 control statements (data residency, TLS 1.3 + HSTS, encryption at rest, auth, audit trail, rule-version pinning, sanctions freshness, retention, deletion, sub-processor change policy, incident response, PII scope), 5-row sub-processor table, attestation roadmap (SOC 2 Type 1 in progress, Type 2 + ISO 27001 not started). Security headers (`Strict-Transport-Security`, `X-Content-Type-Options`, `Referrer-Policy`, `Permissions-Policy`, `X-Frame-Options`) now applied site-wide via `middleware.ts`. SOC2 still pending — track separately. | 🟢 Done (SOC2 still pending) | — |
| 5.5 | ~~No DPIA template~~ **CLOSED 2026-04-29:** `docs/DPIA_TEMPLATE.md` provides 6-section template (processing description, data flow, sub-processors, TOMs Article 32, risks + mitigations, decision) with a fillable answer column. Downloadable from `/trust` via `/api/trust/dpia`. Tests in `tests/trust.test.ts` lock the template structure. | ✅ Closed | — |
...
| 5.7 | ~~No rate-limit on `/api/auth/login`, no MFA~~ **CLOSED 2026-04-29:** Login rate-limit (5/15min/IP+email) at `lib/auth/rate-limit.ts`. **TOTP MFA shipped** in `lib/auth/totp.ts` — RFC 6238 (HMAC-SHA1, 30s step, 6 digits, ±1 step skew, constant-time compare). Endpoints `/api/auth/mfa/{setup,verify,login,disable}`. Login flow issues a 10-minute MFA-pending cookie when TOTP is enrolled; protected pages gated on `mfa: true` claim via middleware + `getAuthenticatedSession()`. Disable requires both password AND current TOTP code. Test `tests/totp.test.ts` includes the RFC 6238 reference-vector match (`287082` at T=59). Note: in-memory rate-limit bucket is per-instance — move to Redis/Vercel KV when scaling beyond one Vercel function. | ✅ Closed | — |
